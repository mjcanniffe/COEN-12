...
//O(n)
//this is a helper function for the quicksort function that helps the function choose a partition to 
//subdivide the array to make sorting easier
static int partition(void *a[], int lo, int high, int (*compare)() ){
	int i;
	int sep;
	void *temp;
	void *x;
...
//this function sorts an array by calling the partition function and recursively sorting smaller and 
//smaller sections of the array
static void quicksort(void *a[], int lo, int high,int (*compare)() ){
	if(lo<high){
		int ploc = partition(a,lo,high,compare);
		quicksort(a,lo,ploc-1,compare );
		quicksort(a,ploc+1,high,compare);
	}
} 
